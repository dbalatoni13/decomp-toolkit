...
where W: Write + ?Sized {
    ...
    for (section_idx, section) in obj.sections.iter() {
        ...
        // Generate local jump labels only within logical code ranges.
        for (range, kind) in obj.sections.kind_ranges_for_section(section_idx) {
            ...
            for (addr, ins) in InsIter::new(&section.data[start..end], range.start) {
                let Some(address) = ins.branch_dest(addr) else { continue };
                if ins.field_aa()
                    || !section.contains(address)
                    || obj.sections.kind_at(SectionAddress::new(section_idx, address))
                        != ObjSectionKind::Code
                {
                    continue;
                }
    ...
    for (section_index, section) in obj.sections.iter() {
        for (reloc_address, reloc) in section.relocations.iter() {
            if !matches!(
                obj.sections.kind_at(SectionAddress::new(section_index, reloc_address)),
    ...
    W: Write + ?Sized,
{
    ...
    loop {
        ...
        let (logical_kind, logical_range) =
            obj.sections.kind_range_at(SectionAddress::new(section_index, current_address));
        if current_address == logical_range.start && current_address != start {
            current_symbol_kind = ObjSymbolKind::Unknown;
            current_data_kind = ObjDataKind::Unknown;
        }
        ...
        } else if logical_kind == ObjSectionKind::Bss {
            writeln!(w, "\t.skip {:#X}", data.len())?;
        } else {
    ...
    W: Write + ?Sized,
{
    ...
    match section.name.as_str() {
        ...
        name => {
            ...
            write!(w, ".section {}", section.name)?;
            match section.kind {
                ObjSectionKind::Code => write!(w, ", \"ax\"")?,
                ObjSectionKind::Data => write!(w, ", \"wa\"")?,
                ObjSectionKind::ReadOnlyData => write!(w, ", \"a\"")?,
                ObjSectionKind::Bss => write!(w, ", \"wa\", @nobits")?,
            }
        }
    };
    if subsection != 0 {
